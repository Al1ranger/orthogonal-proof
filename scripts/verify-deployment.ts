...
const localCode = fs.readFileSync(path.resolve("contracts/orthogonal_proof.py"), "utf8");
...
console.log(JSON.stringify({ status: transaction.statusName, consensus: (transaction as any).result_name,
  contractAddress: address, transactionHash: hash, sourceSha256: sha(localCode),
  schemaMethods: Object.keys((schema as any).methods ?? {}) }, null, 2));
